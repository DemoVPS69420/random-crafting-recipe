            ...
            .then(Commands.literal("shuffle").executes(ctx -> {
                ...
                try {
                    ...
                    src.getServer().getPlayerList().broadcastSystemMessage(
                        Component.literal("§6[RandomCraft] §eRecipes force-shuffled! §7(" + c + " recipes, seed=" + seed + ")"), false);
                    RandomCraftFabric.resetTimer();
                    return c;
                } catch (Throwable t) { src.sendFailure(Component.literal("Shuffle failed: " + t.getMessage())); return 0; }
            }))
            .then(Commands.literal("next").executes(ctx -> {
                long sec = Math.max(0, RandomCraftFabric.ticksUntilNextShuffle() / 20);
