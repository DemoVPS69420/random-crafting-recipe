            ...
            .then(Commands.literal("shuffle").executes(ctx -> {
                ...
                try {
                    ...
                    src.getServer().getPlayerList().broadcastMessage(
                        new TextComponent("§6[RandomCraft] §eRecipes force-shuffled! §7(" + c + " recipes, seed=" + seed + ")"),
                        net.minecraft.network.chat.ChatType.SYSTEM, net.minecraft.Util.NIL_UUID);
                    RandomCraft.resetTimer();
                    return c;
                } catch (Throwable t) { src.sendFailure(new TextComponent("Shuffle failed: " + t.getMessage())); return 0; }
            }))
