...
public class RecipeShuffler {
    ...
    private static Field resultFieldCache = null;

    public static int shuffle(MinecraftServer server, long seed) throws Exception {
        RecipeManager mgr = server.getRecipeManager();
        RegistryAccess access = server.registryAccess();

        Set<ResourceLocation> recipeBlacklist = new HashSet<>();
        for (String s : Config.RECIPE_BLACKLIST.get()) {
            ResourceLocation rl = ResourceLocation.tryParse(s);
            if (rl != null) recipeBlacklist.add(rl);
        }
        Set<ResourceLocation> itemBlacklist = new HashSet<>();
        for (String s : Config.ITEM_BLACKLIST.get()) {
            ResourceLocation rl = ResourceLocation.tryParse(s);
            if (rl != null) itemBlacklist.add(rl);
        }
        boolean includeModded = Config.INCLUDE_MODDED_RECIPES.get();
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            if (trySetResult(recipe, clampCount(newResult.copy()))) {
                changed++;
            }
        }
    ...
    private static void unfinalize(Field f) {
        try {
            java.lang.reflect.Method getDeclaredFields0 = Class.class.getDeclaredMethod("getDeclaredFields0", boolean.class);
            getDeclaredFields0.setAccessible(true);
            Field[] fields = (Field[]) getDeclaredFields0.invoke(Field.class, false);
            for (Field modifiers : fields) {
                if ("modifiers".equals(modifiers.getName())) {
